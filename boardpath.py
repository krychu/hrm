...
def setup_model_and_device(hrm_params: HRMParameters):
    """Setup device and create HRM model."""
...
def run_inference(
        boardpath_params: BoardPathParameters,
        hrm_params: HRMParameters,
        model_path: str = "hrm_boardpath.pt"
):
    ...
    with torch.no_grad():
        ...
        # Run multiple segments (think time)
        for _ in range(hrm_params.infer_segment_cnt):
            z, output_logits = hrm(z, input_flat)
...
def get_config_2():
    ...
    hrm_train_params = HRMTrainParameters(
        train_segment_cnt=2,
        epoch_cnt=100,
        weight_decay=1.0, # 0.01, # default: 0.01, try 0.1
        grad_clip=None, # 1.0
        batch_size=768,
        lr=1e-4 #3e-4
    )
...
if __name__ == '__main__':
    ...
    #set_all_seeds(42)
    boardpath_params, hrm_params, hrm_train_params, train_loader, val_loader = get_config_2()
